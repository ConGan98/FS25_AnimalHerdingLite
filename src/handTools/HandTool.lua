...
function HandTool:loadNonStoreItemAHL(data, xmlFile)
	...
	if self.i3dFilename == nil then
		self:loadFinishedNonStoreItemAHL()
	else

		self:setLoadingStep(SpecializationLoadStep.AWAIT_I3D)
		self.sharedLoadRequestId = g_i3DManager:loadSharedI3DFileAsync(self.i3dFilename, true, false, self.i3dFileLoadedNonStoreItemAHL, self)
...
function HandTool:loadFinishedNonStoreItemAHL()
	...
	if self.loadingState == HandToolLoadingState.OK then
		...
		if self.loadingState == HandToolLoadingState.OK then
			...
			if #self.loadingTasks == 0 then
				self:onFinishedLoadingNonStoreItemAHL()
			else
...
function HandTool:i3dFileLoadedNonStoreItemAHL(node)
...
function HandTool:onFinishedLoadingNonStoreItemAHL()
